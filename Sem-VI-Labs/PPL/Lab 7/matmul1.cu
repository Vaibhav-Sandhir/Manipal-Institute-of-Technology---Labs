// Each row of resultant matrix is computed by one thread
...
__global__ void matmul(int* m1, int* m2, int* m3, int m, int q){
	int rid_m1 = threadIdx.x;
	int sum;
	for(int cid_m2 = 0; cid_m2 < q; cid_m2++){
		sum = 0;
		for(int k = 0; k < m; k++){
...
int main(){
	...
	dim3 dimGrid(1, 1, 1);
	dim3 dimBlock(n, 1, 1);
	matmul <<<dimGrid, dimBlock>>> (cudaM1, cudaM2, cudaM3, m, q);
